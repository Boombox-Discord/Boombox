import { Channel, ClientOptions, Collection, Permissions, TextChannel } from "discord.js";
import { Manager, TrackUtils } from "erela.js";
import Spotify from "erela.js-spotify";
import { lavalinkNodes, spotifyClientID, spotifyClientSecret } from "../config.json";
...
export class Client extends SapphireClient {
    ...
    startManager() {
        ...
        this.manager.on("trackStart", async (player, track) => {
            ...
            if (
                !channel || !this.checkChannelPermissions(channel)
            )
                return;
        ...
        this.manager.on("queueEnd", async (player) => {
            ...
            if (
                !this.checkChannelPermissions(channel)
            ) {
                sendMessage = false;
            }
    ...
    clientEvents() {
        ...
        this.on("interactionCreate", async (interaction) => {
            ...
            if (
                checkChannelPermissions(!interaction.channel)
            ) {
                try {
    ...
    checkChannelPermissions(channel: TextChannel) {
        if (!channel.permissionsFor(this.user.id).has([
