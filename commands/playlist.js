...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const searchEmbed = new Discord.MessageEmbed()
            ...
            .setDescription(
                `Please wait we are searching for that playlist.`
            );
        ...
        if (!response) {
            return message.reply("Sorry, an error has occurred, please try again later!");
        }
        ...
        await getRedis(`guild_${message.guild.id}`, async function(err, reply) {
            ...
            const playlistEmbed = new Discord.MessageEmbed()
                ...
                .setAuthor(message.client.user.username, message.client.user.avatarURL())
            if (errorSongs > 0) {
                playlistEmbed.setDescription(`I have added ${response.tracks.length - errorSongs} to the queue and had an erorr adding ${errorSongs}!`);
            } else {
                playlistEmbed.setDescription(`I have added ${response.tracks.length} to the queue!`);
            }
